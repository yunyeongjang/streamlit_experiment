...
ft1 = f"anthropods/models/feature_columns_bee_venom.json"
cols1 = load_features(ft1)

ft2 = f"anthropods/models/feature_columns_바퀴벌레(꽃가루 제외).json"
...
- 사용자의 알레르기 입력을 바탕으로, 다음 정보를 제공합니다:
  ...
  벌독·바퀴벌레 알러지 위험도를 확률로 예측합니다.''')
    ...
    if (user_name == ''
        or pollen_yesorno == ''
        or (pollen_yesorno == '있음' and general_allergens == [])
        or ('꽃가루' in general_allergens and (pollen_tree == '' or pollen_grass == '' or pollen_weed == ''))):
        st.write('사용자입력 페이지에서 입력을 완료한 뒤 이용해주세요.')

    elif len(general_allergens) == 0:
        st.write('❌ **알레르기 정보가 한정적이라, 알레르기 반응을 예측할 수 없는 상황입니다.**')
        ...
        if '꽃가루' in general_allergens:
            general_allergens2 = general_allergens[:]
            if pollen_grass == '있음':
                general_allergens2.append('풀 꽃가루')
            if pollen_tree == '있음':
                general_allergens2.append('나무 꽃가루')
            if pollen_weed == '있음':
                general_allergens2.append('잔디 꽃가루')
            general_allergens2.remove('꽃가루')

            if len(general_allergens) > 0:
                if '벌독' not in general_allergens:
                    for i in general_allergens2:
                        if i in cols1 and '벌독' not in unknown:
                            unknown.append('벌독')
                else:
                    known.append('벌독')
                if '바퀴벌레' not in general_allergens:
                    for i in general_allergens2:
                        if i in cols2 and '바퀴벌레' not in unknown:
                            unknown.append('바퀴벌레')
                else:
                    known.append('바퀴벌레')
        else:
        ...
        predict_allergen = []
        if len(unknown) <= 1:
            if '벌독' not in unknown and '벌독' not in known:
                st.write('\n')
                st.write('❌ **알레르기 정보가 한정적이라, 알레르기 반응을 예측할 수 없는 상황입니다.**')
                count += 1
                st.markdown(f"""
                    <div style="border: 1px solid #ccc; padding: 10px; border-radius: 8px; background-color: #f9f9f9;">
                        <b>벌독을 분석 위해 필요한 항원 리스트:</b><br>
                        {', '.join(cols1)}
                    </div>
                    """, unsafe_allow_html=True)
            elif '벌독' in unknown:
                st.write('\n')
                st.write('\n')
                if st.session_state["predicted_anthropod"] == False:
                    st.write(f"**벌독**에 대한 알레르기 위험도를 예측하시겠습니까?")
                    # if not st.session_state.get("predicted_anthropod", False):
                    if st.button("예측 실행"):
            ...
            if '바퀴벌레' not in unknown and '바퀴벌레' not in known:
                st.write('\n')
                st.write('\n')
                if count == 0:
                    st.write('❌ **알레르기 정보가 한정적이라, 알레르기 반응을 예측할 수 없는 상황입니다.**')
                st.markdown(f"""
                    <div style="border: 1px solid #ccc; padding: 10px; border-radius: 8px; background-color: #f9f9f9;">
                        <b>바퀴벌레을 분석 위해 필요한 항원 리스트:</b><br>
                        {', '.join(cols2)}
                    </div>
                    """, unsafe_allow_html=True)
            elif '바퀴벌레' in unknown:
                st.write('\n')
                st.write('\n')
                st.write(f"**바퀴벌레**에 대한 알레르기 위험도를 예측하시겠습니까?")
                if st.session_state["predicted_anthropod"] == False:
                    if st.button("예측 실행"):
                        progress_bar = st.progress(0)
                        for i in range(101):
            ...
            st.write(f"**{', '.join(unknown)}**에 대한 알레르기 위험도를 예측하시겠습니까?")
            if st.session_state["predicted_anthropod"] == False:
                if st.button("예측 실행"):
                    progress_bar = st.progress(0)
                    for i in range(101):
        ...
        if st.session_state["predicted_anthropod"]:
            ...
            if "바퀴벌레" in unknown:
                ...
                vec_roach = make_feature_vector("anthropods/models/feature_columns_바퀴벌레(꽃가루 제외).json")
                pred, prob = run_prediction("anthropods/models/model_바퀴벌레(꽃가루 제외).pkl", vec_roach)
            ...
            # 🔁 다시 예측 버튼
            if '바퀴벌레' in unknown or '벌독' in unknown:
                if st.button("초기화"):
    ...
    with tab3:
        ...
        if show_images:
            ...
            if total == 1:
                ...
                with cols[1]:  # 가운데 열
                    st.markdown(
                        f"<div style='text-align: center;'><div style='font-size:14px; font-weight:600; margin-bottom:4px'>{name}</div>",
                        unsafe_allow_html=True)
                    img_data = load_encoded_image(img_path)
                    if img_data:
                        st.markdown(
                            f"<img src='data:image/gif;base64,{img_data}' style='height:180px; width:auto; display:block; margin: 0 auto; object-fit:contain;'>"
                            f"<div style='font-size:11px; color:gray; margin-top:4px; white-space:pre-wrap;'>{source}</div></div>",
                            unsafe_allow_html=True)
                    else:
