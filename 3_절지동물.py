...
general_allergens = st.session_state.get("general_allergens",[])
pollen_tree = st.session_state.get("pollen_tree","")
pollen_grass = st.session_state.get("pollen_grass","")
pollen_weed = st.session_state.get("pollen_weed","")
pollen_yesorno = st.session_state.get("pollen_yesorno","")
...
with tab1:
    st.markdown("<div style='font-size:20px; font-weight:600; margin-bottom:20px;'>🐝 절지동물 알레르기 예측</div>", unsafe_allow_html=True)
    st.markdown('''
- 사용자의 알레르기 입력을 바탕으로, 다음 정보를 제공합니다:
  ...
  벌독·바퀴벌레 알러지 위험도를 확률로 예측합니다.''')
    ...
    # ✅ 사용자 입력 txt 파일에서 불러오기
    def load_user_inputs_from_txt():
        keys = ['user_name', 'general_allergens', 'pollen_tree', 'pollen_grass', 'pollen_weed', 'pollen_yesorno']
        result = {}
        for key in keys:
            path = f"txt/{key}.txt"
            if os.path.exists(path):
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    if key == "general_allergens":
                        result[key] = [x.strip() for x in content.split(",") if x.strip()]
                    else:
                        result[key] = content
            else:
                result[key] = "" if key != "general_allergens" else []
        return result

    user_data = load_user_inputs_from_txt()
    user_name = user_data['user_name']
    general_allergens = user_data["general_allergens"]
    pollen_tree = user_data["pollen_tree"]
    pollen_grass = user_data["pollen_grass"]
    pollen_weed = user_data["pollen_weed"]

    if (user_data['user_name'] == ''
            or (user_data['pollen_yesorno'] == '없음' and user_data['general_allergens'] == '')
            or user_data['pollen_yesorno'] == ''
            or ('꽃가루' in user_data['general_allergens'] and (
                    user_data['pollen_tree'] == '' or user_data['pollen_tree'] == '' or user_data['pollen_tree'] == ''))):
        st.write('---')
    ...
    with tab3:
        ...
        # 엑셀 데이터 로드
        df_bug = pd.read_excel("anthropods/bugs.xlsx")
        ...
        if selected_danger == '왕지네':
            ...
            with col2:
                ...
                st.image(img_path, caption = '출처: OpenAI ChatGPT 이미지 생성 도구 사용(AI가 생성한 가상 이미지입니다)', width=300)  # 또는 height=300
