...
for i in all_list:
    if os.path.exists(f"txt/{i}.txt"):
        with open(f"txt/{i}.txt", "r", encoding="utf-8") as f:
            ...
            if i == "general_allergens":
                all_list[i] = [x.strip() for x in content.split(',') if x.strip()]
            else:
                all_list[i] = content
...
with tab1:
    ...
    if (all_list['user_name'] == ''
        ...
        for pollen in pollens:
            ...
            if '꽃가루' not in all_list['general_allergens']:
                unknown.append(pollen_dic[pollen])
...
with tab2:
    if len(all_list['general_allergens']) > 0 and len(unknown) == 0 or st.session_state["risk_predicted"]:
        ...
        if selected_mountain:
            ...
            if st.button("확인", key="confirm_button"):
                ...
                plant_list = [row[4] for idx, row in df.iterrows() if row[2] == selected_mountain]
