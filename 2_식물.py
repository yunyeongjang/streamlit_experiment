...
with tab1:
    ...
    if (all_list['user_name'] == ''
        ...
        st.write('사용자입력 페이지에서 입력을 완료한 뒤 이용해주세요.')
            ...
            all([all_list.get('pollen_tree'), all_list.get('pollen_grass'), all_list.get('pollen_weed')]) and
            '모름' not in [all_list.get('pollen_tree'), all_list.get('pollen_grass'), all_list.get('pollen_weed')]
    ):
        st.markdown(
        ...
        if (len(all_list['general_allergens']) > 0 and '꽃가루' not in all_list['general_allergens']) \
            ...
            st.write('❌ **알레르기 정보가 한정적이라, 알레르기 반응을 예측할 수 없는 상황입니다.**')
            ft2 = f"plants/models/feature_columns_2nd_없음없음없음.json"
            cols2 = load_features(ft2)
            ft1 = f"plants/models/feature_columns_1st_없음없음.json"
            cols1 = load_features(ft1)
            ...
            st.markdown(f"""
                                            ...
                                            """, unsafe_allow_html=True)
        elif ('꽃가루' in all_list['pollen_yesorno']) and (all_list['pollen_grass'] != '모름' or all_list['pollen_weed'] != '모름') or all_list['pollen_weed'] != '모름':
            pass
        else:
