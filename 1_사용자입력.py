...
if st.session_state['check'] == '':
                ...
                if yn == '없음':
                    ...
                    if gl == []:
                        ...
                        if '꽃가루' not in gl:
                            ...
                            if pt != '' and pg != '' and pw != '':
                                if all(x == '없음' for x in [pt, pg, pw]):
                                    st.markdown('''
                                                <div style='font-size:16px; font-weight:400; line-height:1.5'>
                                                꽃가루 알러지를 선택하셨지만,<br>
                                                세부 항목인 <b>풀 / 나무 / 잔디</b>가 모두 <b>없음</b>으로 설정되어 있습니다.<br><br>
                                                이 경우, 분석에 사용할 데이터가 부족하여 저장이 불가능합니다.<br>
                                                필요한 경우, 꽃가루 항목을 삭제하거나 세부 알러지를 선택해주세요.
                                                </div>
                                            ''', unsafe_allow_html=True)
                                else:
                                    # ✅ 저장 조건 만족
    ...
    if st.button('초기화'):
        ...
        for key in [
            ...
            'saved_user_name', 'saved_pollen_yesorno', 'saved_general_allergens',
            'saved_pollen_tree', 'saved_pollen_grass', 'saved_pollen_weed',
            'check',
            # 👇 추가로 이 3개도 삭제해야 함
            "predicted_anthropod", "pred_bee", "pred_roach",
            "predict_clicked", "risk_predicted", "cols1", "cols2", "prob", "threshold", "unknown_list"
        ]:
    ...
    st.markdown(
        f"<div style='font-size:15px; font-weight:500; color:gray; margin-bottom:15px;'>{st.session_state['saved_user_name']}님, 환영합니다! 😄</div>",
        unsafe_allow_html=True
    )
